...
- Tauri commands registered on the main window: `get_diagnostics`,
  `get_today_activity`, `get_break_summary`, `get_reminder_settings`,
  `get_reminder_status`, `save_reminder_settings`, `reset_reminder_settings`,
  `pause_reminders`, `resume_reminders`, `take_break_now`, `show_overlay_test`,
  `close_overlay_test`, `open_author_website`. Overlay windows only get the
  minimal event/window permissions in `capabilities/overlay.json`.
