...
<script lang="ts">
  ...
  let skipAvailable = $derived(!!layout && !nativeHandoff && skipState === "idle" && preBreakCueSkipAvailable(remainingMs, mode));
  ...
  const visibility = untrack(() => createCueVisibilityController({
    prepare: () => invoke<PreBreakCueLayout>("prepare_pre_break_cue"),
