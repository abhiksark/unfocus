...
- IMPORTANT: probes (idle via XScreenSaver, fullscreen via EWMH) return a
  `Result` per poll. On failure, surface the error string in diagnostics and
  keep the timer running. A probe must never panic or change timer behavior.
