// src-tauri/examples/verify-update-signature.rs
...
fn strict_public_key_text(text: &str) -> Result<(), String> {
    ...
    if !(1..=16).contains(&displayed_id.len())
        || !displayed_id
            .bytes()
    ...
    let packet_id = packet[2..10]
        ...
        .collect::<String>();
    // Tauri can omit leading zeroes; newer minisign also emits zero-padded IDs.
    let unpadded_id = packet_id.trim_start_matches('0');
    let unpadded_id = if unpadded_id.is_empty() {
        "0"
    } else {
        unpadded_id
    };
    if packet_id != displayed_id && unpadded_id != displayed_id {
...
mod tests {
    ...
    #[test]
    fn verifies_tauri_key_with_an_unpadded_id() {
        // Public-only fixture generated by Tauri 2.11.4; the private key was removed.
        const SHORT_KEY: &str = "dW50cnVzdGVkIGNvbW1lbnQ6IG1pbmlzaWduIHB1YmxpYyBrZXk6IDg3QjlGQUM5MTVGOTM0OQpSV1JKazErUnJKOTdDTjV6M0k2bjJCbGdkT2FWWVlKdmlGYThsMlVOOFRsdVhINlVIT3hUcHlNcwo=";
        const SHORT_SIGNATURE: &str = "";
        let (_directory, public_key, signature, data) = fixture();
        fs::write(&public_key, SHORT_KEY).expect("write short-ID public key");
        fs::write(&signature, SHORT_SIGNATURE).expect("write short-ID signature");
        verify_update_signature(&public_key, &signature, &data).expect("valid short-ID signature");
        let text =
            String::from_utf8(STANDARD.decode(SHORT_KEY).expect("decode key")).expect("key UTF-8");
        for id in [
            "0087B9FAC915F9349",
            "87b9fac915f9349",
            "",
            ...
            assert!(strict_public_key_text(&text.replace("87B9FAC915F9349", id)).is_err());
        }
        strict_public_key_text(&text.replace("87B9FAC915F9349", "087B9FAC915F9349"))
            .expect("valid padded ID");
        fs::write(&data, b"tampered").expect("tamper fixture");
        assert!(verify_update_signature(&public_key, &signature, &data).is_err());
    }
